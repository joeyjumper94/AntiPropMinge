...
if SERVER then
	...
	concommand.Add("apm_enable_fall_damage", function(ply)
		if ply:IsValid() and !ply:IsSuperAdmin() then
			ply:PrintMessage(HUD_PRINTTALK, "you need to be a superadmin to do that")
			return
		end
		APM_Convar_MSG="fall damage is now enabled"
		apm_tab:EasyFPrint(APM_Convar_MSG,0)
		print(APM_Convar_MSG)
		RunConsoleCommand("apm_allow_world_damage",1)
	end)
	concommand.Add("apm_disable_fall_damage", function(ply)
		if ply:IsValid() and !ply:IsSuperAdmin() then
			ply:PrintMessage(HUD_PRINTTALK, "you need to be a superadmin to do that")
			return
		end
		APM_Convar_MSG="fall damage is now disabled"
		apm_tab:EasyFPrint(APM_Convar_MSG,0)
		print(APM_Convar_MSG)
